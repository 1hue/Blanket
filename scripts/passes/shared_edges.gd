...
func init_uniforms() -> void:
	...
	shared_edge_uniform_set = rd.uniform_set_create([
		...
		ComputeUtil.create_uniform([shared_mask_buffer], RenderingDevice.UNIFORM_TYPE_STORAGE_BUFFER, 1),
...
func _notification(what) -> void:
	if what != NOTIFICATION_PREDELETE:
		return
	for rid in [
		shared_edge_uniform_set, shared_edge_buffer, shared_mask_buffer,
