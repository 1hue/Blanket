...
func init_bevel_fill_dispatch() -> void:
	bevel_fill_dispatch_buffer = dispatch_buffer_create()

	bevel_fill_dispatch_set = rd.uniform_set_create([
		ComputeUtil.create_uniform([bevel_fill_dispatch_buffer], RenderingDevice.UNIFORM_TYPE_STORAGE_BUFFER),
	], SurfaceShaders.shared_edges.shader, 3)

	sets.bevel_fill_dispatch_buffer = bevel_fill_dispatch_buffer
...
func compute() -> void:
	...
	rd.buffer_clear(bevel_fill_dispatch_buffer, 0, 12)

	var compute_list := rd.compute_list_begin()
	rd.compute_list_bind_compute_pipeline(compute_list, SurfaceShaders.shared_edges.pipeline)
	rd.compute_list_set_push_constant(compute_list, pack_params(), SIZE_PARAMS)
	rd.compute_list_bind_uniform_set(compute_list, sets.faces_out, 0)
	...
	rd.compute_list_bind_uniform_set(compute_list, bevel_fill_dispatch_set, 3)
...
func _notification(what) -> void:
	...
	for rid in [
		...
		bevel_fill_dispatch_set, bevel_fill_dispatch_buffer,
	]:
		if rid.is_valid():
			rd.free_rid(rid)
