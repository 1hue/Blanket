...
func pack_params() -> PackedByteArray:
	push_constant.encode_u32(0, params.faces_out_marker_offset)
...
func compute() -> void:
	...
	rd.buffer_clear(attribute_buffer, 0, params.faces_out_vertex_count * params.faces_out_attribute_stride)

	var compute_list := rd.compute_list_begin()
	rd.compute_list_bind_compute_pipeline(compute_list, SurfaceShaders.faces_pin.pipeline)
	rd.compute_list_set_push_constant(compute_list, pack_params(), SIZE_PARAMS)
	rd.compute_list_bind_uniform_set(compute_list, uniforms.faces_out_set, 0)
