...
func compute() -> void:
	...
	rd.compute_list_bind_compute_pipeline(compute_list, BlanketShaders.shrink.pipelines[version])
	...
	rd.compute_list_bind_uniform_set(compute_list, sets.face_edge, 2)
